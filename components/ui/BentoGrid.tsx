...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    const text = "aabdalnaby073@gmail.com";
    ...
    >
      ...
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        <div className= {cn(titleClassName , 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10')}>
            <div className="font-sans font-extralight text-[#c1c2d3] text-sm md:text-xs lg:text-case z-10 dark:text-neutral-300">
              {description}
            </div>
        ...
        {id ==3 && (
          <div className='flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2'>
            <div className='flex flex-col gap-3 lg:gap-4'>
              {['Css' ,  'React.js' , 'Express' , 'Node.js' , 'Typrscript'].map( (item , i) =>
                (<span
                    key={i}
            ...
            <div className='flex flex-col gap-3 lg:gap-4'>
              <span className="lg:py-4 lg:px-3 py-4 px-3  rounded-lg text-center bg-[#10132E]"></span>
              {['HTML','Mongoose' , 'MongoDB' , 'SQL' , 'Javascript'].map( (item , i) =>
                (<span
                    key={i}
                    className="lg:py-4 lg:px-3 py-2 px-3 text-xs lg:text-base opacity-50 
                    lg:opacity-100 rounded-lg text-center bg-[#10132E]"
